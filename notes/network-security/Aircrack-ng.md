...
6. **aireplay-ng -0 0 -a MAC_AP -c MAC_Client wlan0mon**

This command uses aireplay-ng to transmit deauthentication frames to a wireless client and because these frames are not authenticated in legacy WiFi configurations, they can be forged and transmitted by an external device.

*- 0* specifies the attack mode in this case deauthentication management frames (check for other attacks in the official site) and *0* send deauthentication frames continuously.


7. **aircrack-ng file.pcap -w /opt/wordlist.txt**
